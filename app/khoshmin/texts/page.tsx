...
export default function TextsManager() {
  ...
  // state های مربوط به آپلود لوگو (با آپلود معوق)
  ...
  const confirmCrop = async () => {
    ...
    let croppedBlob: Blob = selectedLogoFile;
    if (cropRect.w > 10 && cropRect.h > 10) {
      ...
      if (imgElem) {
        ...
        if (ctx) {
          ...
          croppedBlob = await new Promise<Blob>(resolve => canvas.toBlob(b => resolve(b!), selectedLogoFile.type, 0.9));
        }
      }
    }
    ...
    showAlert('تصویر با موفقیت برش خورد', '', 'success');
  };
  ...
  const handleDeleteLogo = () => {
    ...
    if (croppedLogoBlob) {
      showConfirm({
        ...
        message: 'آیا از حذف لوگوی جدید (ذخیره نشده) اطمینان دارید؟',
    ...
    if (texts.FOOTER_LOGO) {
      showConfirm({
        title: 'حذف لوگو',
        message: 'آیا از حذف لوگوی فعلی اطمینان دارید؟ این عمل غیرقابل بازگشت است.',
        confirmText: 'بله، حذف شود',
        cancelText: 'انصراف',
        type: 'error',
        onConfirm: async () => {
          try {
            ...
            markUnsaved();
            showAlert('لوگو با موفقیت حذف شد', '', 'success');
          } catch (e) {
  ...
  // ذخیره نهایی با آپلود لوگو در صورت وجود
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!validatePhoneEmail()) {
      showAlert('لطفاً اطلاعات شماره تماس و ایمیل را اصلاح کنید.', 'خطا', 'error');
      return;
    }
    setSaving(true);
    try {
                ...
                <div className="bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                      ...
                      {croppedLogoBlob || logoPreviewUrl ? (
                        <div className="flex flex-col xl:flex-row items-center gap-8 p-6 border-2 border-blue-200 bg-blue-50/50 rounded-2xl">
                          <div className="w-full xl:w-2/3 bg-gray-100 border-2 border-dashed border-gray-300 rounded-xl p-4">
                            <div className="relative inline-block cursor-crosshair" onMouseDown={onCropMouseDown} onMouseMove={onCropMouseMove} onMouseUp={onCropMouseUp} onMouseLeave={onCropMouseUp}>
                              <img id="crop-source-img" src={logoPreviewUrl!} alt="Preview" className="max-w-full max-h-[300px] object-contain pointer-events-none" draggable={false} />
                          ...
                          <div className="flex flex-col gap-4 w-full xl:w-1/3">
                            <button type="button" onClick={confirmCrop} disabled={uploadingLogo} className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <input
                        type="text"
                        value={metaTitle}
                        onChange={(e) => { setMetaTitle(e.target.value); markUnsaved(); }}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => { setMetaDescription(e.target.value); markUnsaved(); }}
                        rows={3}
                        className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-green-500 outline-none bg-white text-gray-900"
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={metaKeywords}
                        onChange={(e) => { setMetaKeywords(e.target.value); markUnsaved(); }}
                        className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-green-500 outline-none bg-white text-gray-900"
