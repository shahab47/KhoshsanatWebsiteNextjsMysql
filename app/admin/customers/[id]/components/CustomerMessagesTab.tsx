// app/admin/customers/[id]/_components/CustomerMessagesTab.tsx
'use client';
...
export function CustomerMessagesTab({ customerId }: { customerId: string }) {
  ...
  const updateAdminReply = async (messageId: number, newReply: string) => {
    setSubmitting(true);
    try {
      const res = await fetch(`/api/admin/customers/${customerId}/messages?messageId=${messageId}`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        alert(error.error || 'خطا در ذخیره پاسخ');
      }
    } catch {
  ...
  if (loading) {
    return <div className="text-center py-8">در حال بارگذاری پیام‌ها...</div>;
  }

  if (messages.length === 0) {
    return <p className="text-gray-500 text-center py-8">پیامی وجود ندارد</p>;
  }
