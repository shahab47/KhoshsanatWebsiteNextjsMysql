...
export default function LoginPage() {
  ...
  const [showPassword, setShowPassword] = useState(false); // 🟢 استیت نمایش پسورد
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      if (action === 'login') {
        // تغییر به replace تا کش صفحات پاک شود
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {view !== 'forgot' && (
            <div className="relative flex items-center">
              <KeyRound className="absolute right-4 text-gray-400" size={20} />
              <input 
                type={showPassword ? 'text' : 'password'} // 🟢 سوئیچ نوع اینپوت
                placeholder={view === 'reset' ? "رمز عبور جدید" : "رمز عبور"} 
                required 
                className="w-full bg-gray-50 border border-gray-200 rounded-xl py-4 pr-12 pl-12 outline-none focus:ring-2 focus:ring-blue-500" 
                value={password} 
