...
export async function GET() {
  ...
  if (session.role === 'MAIN_ADMIN') {
    const users = await db.user.findMany({
      select: { id: true, name: true, email: true, role: true, allowedPaths: true }
    });
...
export async function POST(req: Request) {
  ...
  const { name, email, password, role, allowedPaths } = await req.json();
  const hash = await bcrypt.hash(password, 10);
  try {
    await db.user.create({
      data: { name, email, password: hash, role, allowedPaths: allowedPaths || null }
    });
...
export async function PUT(req: Request) {
  ...
  const { id, name, email, password, role, allowedPaths } = await req.json();
