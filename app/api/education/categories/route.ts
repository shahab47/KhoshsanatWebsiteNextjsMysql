...
export async function POST(request: NextRequest) {
  try {
    ...
    const exists = await db.articleCategory.findUnique({ where: { title } });
    if (exists) {
      return NextResponse.json({ error: 'این دسته‌بندی قبلاً وجود دارد' }, { status: 400 });
    }
    const category = await db.articleCategory.create({ data: { title } });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // بررسی وجود دسته قدیم
    const existing = await db.articleCategory.findUnique({ where: { title: oldTitle } });
    if (!existing) {
      return NextResponse.json({ error: 'دسته‌بندی مورد نظر یافت نشد' }, { status: 404 });
    }
    // بررسی نبودن تکراری برای عنوان جدید
    const duplicate = await db.articleCategory.findUnique({ where: { title: newTitle } });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const category = await db.articleCategory.findUnique({ where: { title } });
    if (!category) {
      return NextResponse.json({ error: 'دسته‌بندی یافت نشد' }, { status: 404 });
    }
    await db.articleCategory.delete({ where: { title } });
