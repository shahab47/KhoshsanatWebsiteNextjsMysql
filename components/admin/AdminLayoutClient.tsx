...
function AdminLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  const getBreadcrumbs = (): { name: string; href: string }[] => {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      let name = '';
      switch (segment) {
        case 'analytics': name = 'آمار و تحلیل بازدید'; break;
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              <button onClick={() => setIsMobileMenuOpen(true)} className="xl:hidden p-2 text-gray-300 hover:text-white transition cursor-pointer">
                <Menu size={24} />
              </button>
          ...
          >
            ...
            <div className="mt-6 pt-6 border-t border-slate-700 flex flex-col gap-3">
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-bold text-red-100 bg-red-600/20 hover:bg-red-600 hover:text-white transition-colors cursor-pointer"
              >
